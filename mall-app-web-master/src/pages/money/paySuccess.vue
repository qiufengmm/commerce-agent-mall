...
import { PAY_TEXT_FAILED, PAY_TEXT_SUCCESS, resolvePayResultText } from '@/utils/payment'
...
// ===== 数据加载 =====
// 查询支付宝交易状态，状态映射统一由 resolvePayResultText 处理（见 src/utils/payment.ts）
const loadData = async (outTradeNo: string) => {
  try {
    const res = await fetchAliapyStatusAPI({ outTradeNo })
    payText.value = resolvePayResultText(res.data)
  } catch (e) {
    console.error('查询支付宝状态失败', e)
    payText.value = PAY_TEXT_FAILED
  }
}
...
onLoad((options: any) => {
  if (!USE_ALIPAY) {
    payText.value = PAY_TEXT_SUCCESS
    return
  }
