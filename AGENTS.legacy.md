...
3. 通过 WorkBuddy MCP 调度 CodeBuddy 实现（CodeBuddy 承担实际编码，WorkBuddy MCP 只是调度层），提示词必须包含已确认的设计、文件范围、禁止扩大范围与验证要求；
4. 读取通过 WorkBuddy MCP 返回的 CodeBuddy 修改文件、位置、`session` 和 `[workbuddy meta]`，但一律视为草稿；
5. **成本透明（强制步骤）**：每次收到 CodeBuddy 返回结果后，主 Agent 必须在给用户的进度报告/最终报告中展示本次调用的：
   - 模型、积分、输入 Token、输出 Token、耗时、回合数、`session`；
   - 任一字段缺失时，必须明确标注"不可用"，不得猜测、不得沿用上次数值、不得只写"见日志"；
   - 同时必须检查返回末尾的 `[workbuddy meta]`：**仅当单次 CodeBuddy 调用的 `credit` 超过 20 时**，才停止无目标的续接（`resumeSessionId`），先向用户说明该 credit 数值，再把任务拆小（更小的任务范围）后继续；不得因为 Token 数量、耗时或 turns 单独触发该限制；不要要求或建议设置 `WB_MAX_TURNS`。
6. 主 Agent 用 `git diff` 核对实际改动，再交由 `reviewer` 只读审查；
7. 若审查存在 P0/P1 或未满足验收标准，优先携带原 `resumeSessionId` 让同一 WorkBuddy 会话返工；
8. 审查通过后，运行与改动相关的测试、构建或接口验证；验证通过后，复核 `git status` 与暂存区，确认只包含本次任务文件；
9. 使用中文 Git 提交信息创建本地提交，并向用户报告提交哈希、文件清单与验证结果；
10. 只有在用户明确确认该提交无误后，才执行 `git push` 上传 GitHub 仓库；推送后再次报告远端分支与提交哈希。
...
- **成本可见**：返回文本末尾附 `[workbuddy meta] model=... credit=... in=... out=...`，须按「标准实现闭环」第 5 步在报告中逐项展示（缺失标"不可用"）；仅当单次调用的 `credit` 超过 20 时先停手说明并拆小任务，不得无目标续接；不得因 Token 数量、耗时或 turns 单独触发该限制，也不要要求或建议设置 `WB_MAX_TURNS`。
- **固定可信 CLI**：驱动 CLI 被 Server 固定为硬编码路径（`D:\nodejs\npm_global\codebuddy.cmd`）与固定受信版本，不接受 `WB_COMMAND` 等环境变量覆盖；每次任务启动模型前都会复核路径与 `--version`，不符即拒绝。CodeBuddy 升级后不会自动放行，需人工完成只读行为验证后再更新 Server 中的受信版本常量。
