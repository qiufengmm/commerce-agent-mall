...
### 5.1 本地局域网最终验证记录（2026-09-21）

本次使用当前 `.env` 的局域网配置执行只读验证，未执行 SQL 写操作、MinIO 上传、删除或策略修改：
...
| 数据库中有图片引用的商品数 | `37` |
| 数据库中不同商品主图路径 | `22` |
| 不同商品主图通过 `http://192.168.85.211:9000` 验证 | `22/22` 返回 `200` |
| 代表对象 | `images/20180522/web.png`，`200 image/png`，`3288` bytes |
| MinIO 匿名策略 | `download` |
| H5 入口 | `200 text/html` |
| 门户搜索 `/portal-api/product/search?pageNum=1&pageSize=1` | `200 application/json` |
| MinIO CORS 预检与真实 GET | `204` / `200`，允许 Origin 为 `http://192.168.85.211:8088` |

本次 H5 使用以下构建参数重新生成产物，Nginx 通过宿主机目录挂载直接读取新产物：

```powershell
$env:VITE_API_BASE_URL = '/portal-api'
$env:VITE_MINIO_PUBLIC_ENDPOINT = 'http://192.168.85.211:9000'
npm run build:h5
```

构建产物已确认不含 `localhost:8085`、旧局域网 IP 和旧通知图片文件名；
`notice-banner-01.jpg`、`notice-banner-02.jpg` 两种路径（`/static/**`、`/h5/static/**`）均返回 `200 image/jpeg`。

---

## 6. 遗留项

- MinIO 真实商品对象、匿名下载、CORS、H5 入口、门户搜索和两张通知图已完成宿主机局域网验证；
- 物理手机上用夸克重新打开通知页、确认广告过滤开启时新文件名仍显示，以及微信开发者工具真机图片验证仍需设备侧执行；
- 微信开发者工具安全域名配置仍需在实际微信开发者工具/小程序环境中确认；
- 资源改名后的夸克浏览器页面内最终验收已具备新产物，仍需在物理手机上执行最后复测；
