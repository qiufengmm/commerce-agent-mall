...
- [ ] 在具备运行环境后验证首页、分类、商品详情、评价、购物车和订单图片均返回 200。
- [ ] 在具备设备后验证字体、MinIO 公开读、跨域和微信开发者工具安全域名配置。
- [x] 独立提交：`7b9a5e2 完善移动端 MinIO 局域网图片访问`，已快进合并到 `main`。

代码侧验证已完成：移动端 Vitest 63 个用例、TypeScript 检查、H5 构建和微信小程序构建均通过；当前没有可用的真机/微信开发者工具，且本轮未进行设备级 HTTP 200 验证。
