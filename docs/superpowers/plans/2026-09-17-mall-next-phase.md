...
- [x] 根据用户确认，已完成微信开发者工具中的商品图片与通知页最终确认；正式 HTTPS 合法域名配置按后续环境任务处理。
- [x] 独立提交：`7b9a5e2 完善移动端 MinIO 局域网图片访问`，已快进合并到 `main`。

代码侧验证已完成：移动端 Vitest 63 个用例、TypeScript 检查、H5 构建和微信小程序构建均通过；宿主机局域网 H5/Nginx、MinIO 真实商品对象、匿名读和 CORS 已实测；微信开发者工具图片验收已由用户确认完成。
